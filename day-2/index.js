...
const main = async () => {
  ...
  const calcScore = (opponent, me) => {
    ...
    const score = bonusPoints[me] + roundResultPoints[opponent][me] * 3;

    return score;
  };

  const calcGuidedScore = (opponent, me) => {
    const roundResultPoints = {
      A: {
        X: 3,
        Y: 4,
    ...
    return roundResultPoints[opponent][me];
  };

  const guessedScore = lines.reduce((acc, round) => {
    const [opponent, me] = round.split(" ");
    const score = calcScore(opponent, me);
    return acc + score;
  }, 0);

  const guidedScore = lines.reduce((acc, round) => {
    const [opponent, me] = round.split(" ");
    const score = calcGuidedScore(opponent, me);
    return acc + score;
  }, 0);

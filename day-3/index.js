...
const main = async () => {
  ...
  console.log("Group badge sum:", calcGroupBadgeSum(lines));
};

const calcTotalPriority = (lines) => {
  const getPriorityForLine = (line) => {
    const firstHalf = new Set(line.slice(0, line.length / 2));
    const secondHalf = new Set(line.slice(line.length / 2));

    const intersection = [...firstHalf].filter((item) => secondHalf.has(item));

    return getPriority(intersection[0]);
  };

  return lines.reduce((acc, line) => acc + getPriorityForLine(line), 0);
};

const calcGroupBadgeSum = (lines) => {
  let sum = 0;
  for (let i = 0; i < lines.length; i += 3) {
    const first = new Set(lines[i]);
    const second = new Set(lines[i + 1]);
    const third = new Set(lines[i + 2]);

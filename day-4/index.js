...
const main = async () => {
  ...
  console.log("Total overlap count:", countTotalOverlaps(lines));
  console.log("All overlap count:", countAllOverlaps(lines));
};

const countTotalOverlaps = (lines) => {
...
const getNumsFromLine = (line) => {
  const [first, second] = line.split(",");
  const [firstStart, firstEnd] = first.split("-");
  const [secondStart, secondEnd] = second.split("-");

  const firstStartNum = Number(firstStart);
  const firstEndNum = Number(firstEnd);
  const secondStartNum = Number(secondStart);
  const secondEndNum = Number(secondEnd);

  return [firstStartNum, firstEndNum, secondStartNum, secondEndNum];
};

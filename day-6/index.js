...
const main = async () => {
  ...
  console.log("Start marker after:", findStartMarker(lines[0]));
  console.log("Message marker after:", findMessageMarker(lines[0]));
};

const findStartMarker = (line) => {
  for (let i = 3; i < line.length; i++) {
    const signal = new Set([line[i - 3], line[i - 2], line[i - 1], line[i]]);
    if (signal.size === 4) return i + 1;
  }
};

const findMessageMarker = (line) => {
  for (let i = 14; i < line.length; i++) {
    const signal = new Set(line.slice(i - 14, i));
    if (signal.size === 14) return i;
  }
};

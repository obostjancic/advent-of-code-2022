...
const MULTI_MOVE = true;

const main = async () => {
  const lines = await readInput(__dirname);
  const delimiterIdx = lines.indexOf("");
  const containers = lines.slice(0, delimiterIdx).reverse();

  const moves = lines.slice(delimiterIdx + 1);

  console.log(
    "Tops of stacks:",
    getTopsOfStacks(initStacks(containers), moves, !MULTI_MOVE)
  );
  console.log(
    "Tops of stacks:",
    getTopsOfStacks(initStacks(containers), moves, MULTI_MOVE)
...
const initStacks = (crateRows) => {
  const [indices, ...rows] = crateRows;
  const numOfStacks = indices.split(" ").filter((item) => item !== "").length;

  const stacks = Array.from({ length: numOfStacks }, () => []);

  rows.forEach((crateRow) => {
...
const getTopsOfStacks = (stacks, moves, withMultiMove = false) => {
  playMoves(moves, stacks, withMultiMove);

  const topsOfStacks = stacks.map((stack) => stack[stack.length - 1]).join("");
  return topsOfStacks;
};

const playMoves = (moves, stacks, multiMoveEnabled = false) => {
  moves.forEach((move) => {
